...
export default class DateInterval {
  ...
  getFormattedDate (format) {
    ...
    return {
      from: moment(this.from).format(format),
      to: moment(this.to).format(to)
    };
  }
  getQuery (format, isNumber) {
    if(!this.hasRange()) {
      return;
    }
    let from = moment(this.from).format(format);
    let to = moment(this.to).format(format);
    if(!isNaN(from) && isNumber !== false) {
      from = parseInt(from);
      to = parseInt(to);
    }
    if(from === to) {
