...
export let tkgConst = {
  ...
  getDateRange: {
    ...
    monthToDate: (isToday, momentObj) => {
      var date = moment();
      var fromDate = date.clone().startOf('month');
      var toDate = isToday ? date : date.subtractWeekDays(1, 'day').endOf('day');
      if(fromDate.isDST()) {
        fromDate.add(1, 'hour');
      }
      if(momentObj) {
    ...
    quarterToDate: (isToday, momentObj) => {
      let date = moment();
      // let quarter = date.quarter();
      let fromDate = date.clone().startOf('quarter');
      let toDate = isToday ? date : date.subtractWeekDays(1, 'day').endOf('day');
      if(fromDate.isDST()) {
        fromDate.add(1, 'hour');
      }
      if(momentObj) {
