...
export function verticalGrid () {
  return {
    restrict: 'E',
    template: require('./vertical-grid.jade'),
...
class vGridCtrl {
  constructor ($scope, $attrs, tkgConfig, prepareWindowEvents) {
    this.$scope = $scope;
    let from = $attrs.from;
    this.templates = from ? tkgConfig.getGridListObj()[from] : tkgConfig.getGridList();
  ...
  initialize (extraDelay = 0) {
    ...
    this.initHeightTimer = setTimeout(() => {
      ...
      let nonZeroChildren = _.filter(this.children, (child) => {
        return child.tmpl.height !== 0;
      });
...
export function vGridMaximizer () {
  return {
    restrict: 'E',
    template: require('./vertical-grid-maximizer.jade'),
    link: ($scope, element, attrs) => {
