export function tkgSingleGrid (tkgConfig) {
  return {
    restrict: 'E',
    template: require('./single-grid.jade'),
...
export function singleGrid ($timeout) {
  return {
    restrict: 'C',
    link: function ($scope, element, attrs) {
      ...
      tkgConst.prepareWindowEvents($scope, adjustLastChildHeighht);
    }
  };
}

export function singleGridChildren ($animate, $rootScope, $interpolate) {
  return {
    restrict: 'A',
    priority: 0,
    ...
    link: function ($scope, element, attrs, ctrl) {
      ...
      if(gridHeight)  {
        if(/%/.test(gridHeight)) {
          ...
          tkgConst.prepareWindowEvents($scope, adjustHeight);
          setTimeout(function () {
            adjustHeight();
          }, 200);
        } else {
...
export function singleGridMinimizer ($parse) {
  return {
    restrict: 'E',
...
export function singleGridMaximizer ($rootScope) {
  return {
    restrict: 'E',
    template: require('./single-grid-maximizer.jade'),
